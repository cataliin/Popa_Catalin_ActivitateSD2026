...
Heap citireMasiniDinFis(const char* numeFis) {
...
Masina extrageMasina(Heap* heap) {
	...
	if (heap->nrMasini > 0) {
		...
		heap->nrMasini--;
		filtreazaHeap(*heap, 0); // nu trebuie for, deoarece avem heap valid in rest si nu trebuie schimbat
	}
	return m;
}

int main() {
	Heap heap = citireMasiniDinFis("masini.txt");
	afisareHeap(heap);
	return 0;
}